...
toggle.addEventListener('click', () => {
  ...
  toggle.src = hidden ? '../images/icon_eye_on.png' : '../images/icon_eye_off.png';
});
...
function  disableButton() {
  if(loginButton) {
    ...
    const errorMassage = inputs.some(i => i.value.trim() === '' || i.nextElementSibling.classList.contains('active'));
    loginButton.disabled = errorMassage;
    loginButton.style.backgroundColor = errorMassage ? '#9CA3AF' : '#3692FF'
    loginButton.style.cursor = errorMassage ? 'not-allowed' : 'pointer';
  }
...
// 회원가입 버튼
  if (signupButton) {
    ...
    const errorMassage = inputs.some(i => i.value.trim() === '' || i.nextElementSibling.classList.contains('active'));
    signupButton.disabled =errorMassage;
    signupButton.style.backgroundColor = errorMassage ? '#9CA3AF' : '#3692FF'
    signupButton.style.cursor = errorMassage ? 'not-allowed' : 'pointer';
  }
}
